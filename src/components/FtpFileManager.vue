...
export default {
  ...
  methods: {
    ...
    handleNewOk() {
      ...
      createDirectory(this.client, this.path + "/" + this.filename)
        .then(() => {
          // 成功时关闭弹窗关重渲染页面
          // 使用正则表达式判断不能全为空格
      ...
      if (row.isDirectory) {
        ...
        if (
          ...
          if (row.size <= 2000000) {
            ...
            this.dialogWidth = "30%";
            getFileContents(this.client, {}, row.path)
              .then((content) => {
                console.log(content);
    ...
    handleDownloadClick() {
      ...
      downloadFile(this.client, this.selectRow.path).then((res) => {
        console.log(res);
        console.log("下载成功");
        alert("下载成功");
      });
    ...
    handleFileRender() {
      ...
      // 获取目录文件
      getDirectoryContents(this.client, {}, this.path)
        .then((res) => {
          console.log(res);
          this.fileItems = [];
