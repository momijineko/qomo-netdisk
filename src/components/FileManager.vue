...
import { create, getQuota } from "./../apis/netdisk";
import {
...
export default {
  ...
  data: () => {
    return {
      ...
      dlBtn: true, // 下载是否可用按钮，默认为不可用，因为默认进入的是本地文件
  ...
  methods: {
    ...
    handleNewOk() {
      ...
      createDirectory(this.client, this.path + "/" + this.filename, "0")
        .then(() => {
          // 成功时关闭弹窗关重渲染页面
          // 使用正则表达式判断不能全为空格
    ...
    handleOpenClick(row) {
      ...
      if (row.isDirectory) {
        this.prevPath.push(this.path);
        this.$store.commit("setPath", (this.path = row.path));
        this.handleFileRender();
      } else {
        ...
        if (
          ...
          if (row.size <= 2000000) {
            this.dialog = true;
            this.dialogTitle = "预览";
            this.dialogInfo = "文件加载中，请稍等……";
            this.dialogWidth = "30%";
            getFileContents(this.client, {}, row.path, "0")
              .then((content) => {
    ...
    handlePasteClick() {
      ...
      if (this.$store.state.mvpath.type == 1) {
        copyFile(
          ...
          this.selectRow.isDirectory
            ? this.selectRow.path +
                "/" +
                path.basename(this.$store.state.mvpath.path)
            : path.dirname(this.selectRow.path) +
                "/" +
                path.basename(this.$store.state.mvpath.path),
          "0"
        )
    ...
    handleRenameOk() {
      ...
      moveFile(
        this.client,
        this.selectRow.path,
        path.dirname(this.selectRow.path) + "/" + this.filename,
        "0"
      )
        .then((res) => {
          // 成功时关闭弹窗关重渲染页面
    ...
    handleDeleteOk() {
      deleteFile(this.client, this.selectRow.path)
        .then((res) => {
          console.log(res);
    ...
    handleFileRender() {
      ...
      // 获取目录文件
      getDirectoryContents(this.client, {}, this.client.path, "0")
        .then((res) => {
          this.fileItems = [];
          ...
          // 存储路径时顺便格式化
          res.forEach((item) => {
            if (item.isDirectory() && item["Symbol(type)"] != 3) {
              this.fileItems.push({
                name: item.name,
                isDirectory: item.isDirectory(),
                path: path.normalize(this.path + "/" + item.name),
              });
              this.directoryCount++;
            }
          });
          res.forEach((item) => {
            if (item.isFile() && item["Symbol(type)"] != 3) {
              this.fileItems.push({
                name: item.name,
                isDirectory: item.isDirectory(),
                path: path.normalize(this.path + "/" + item.name),
          ...
          this.fileItems.forEach((item) => {
            getFileStat(this.client, this.client.path + "/" + item.name, "0")
              .then((stat) => {
                if (!item.isDirectory) this.$set(item, "size", stat.size);
                // 使用第三方库date-format
                this.$set(
                  item,
                  "birthDate",
                ...
                this.$set(
                  item,
                  "changeDate",
                  format.asString("yyyy-MM-dd hh:mm:ss", new Date(stat.ctime))
                );
                this.$set(
                  item,
                  "accessDate",
                  format.asString("yyyy-MM-dd hh:mm:ss", new Date(stat.mtime))
                );
                // item.birthDate = stat.birthtimeMs;
                // item.changeDate = stat.ctimeMs;
              })
              .catch((err) => err);
          });
        })
  ...
  beforeMount() {
    ...
    this.client = create({
      // 根路径
      remoteURL: os.homedir(),
      // remoteURL: ''
      type: 0,
    });
    getQuota(this.client, "0").then((res) => {
      let data = [];
      console.log(res);
      let count = 0;
      res.forEach((item) => {
        // if (item.target == '/' || os.platform() == 'win32')
        data.push({
          id: count++, // 用于循环key值
          name: item.target,
          // path: item.target,
          used: filesizeConvert(item.used),
          total: filesizeConvert(item.size),
          // 当获取的结果如果没有百分号时自动添加百分号
          pcent: item.pcent.indexOf("%") == -1 ? item.pcent + "%" : item.pcent,
        });
      });
      console.log(data);
      this.$store.commit("setDiskInfo", data);
    });
    // os.homedir() os模块的homedir方法， 跨平台获取家目录
    // this.$store.commit("setPath", os.homedir());
    if (this.$route.hash == "") {
      this.$store.commit("setPath", os.homedir());
    } else {
      this.$store.commit(
        "setPath",
        path.normalize(
          this.$store.state.diskInfo[this.$route.hash.slice(1)].name + "/"
        )
      );
    }
    // 调用文件列表渲染函数
