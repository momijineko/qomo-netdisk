...
export default {
  ...
  methods: {
    ...
    handleNewOk() {
      ...
      createDirectory(this.client, this.path + "/" + this.filename, "2")
        .then(() => {
          // 成功时关闭弹窗关重渲染页面
          // 使用正则表达式判断不能全为空格
    ...
    handleDownloadClick() {
      ...
      downloadFile(this.client, this.selectRow.path, "2").then((res) => {
        console.log(res);
        console.log("下载成功");
        alert("下载成功");
      });
    ...
    handleDeleteOk() {
      deleteFile(this.client, this.selectRow.path, "2")
        .then((res) => {
          console.log(res);
    ...
    handleFileRender() {
      ...
      // 获取目录文件
      getDirectoryContents(this.client, {}, this.path, "2")
        .then((res) => {
          console.log(res);
          this.fileItems = [];
